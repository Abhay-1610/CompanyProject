...
function SuperAdminChangeHistory() {
  const [list, setList] = useState([]);
  const [loading, setLoading] = useState(false);

  /* =============================
     FETCH ALL AUDIT HISTORY
     (SUPER ADMIN)
  ============================== */
  useEffect(() => {
    const fetchChangeHistory = async () => {
      try {
        setLoading(true);
        const res = await baseUrl.get("/change-history/all");
        setList(res.data);
      } catch (err) {
    ...
    >
      ...
      <div className="mb-4">
        ...
        <p className="text-muted mb-0">
          Audit log of all activities across all companies
        </p>
      </div>
...
export default SuperAdminChangeHistory;
